...
public class Master implements Observer{
    ...
    public void receiveOffer(String nameCompany, int salary, Vacancy vacancy) {
        if (this.salary <= salary){
            System.out.printf("Специалист %s: мне нужна эта работа! компания %s; вакансия %s; зар.плата %d)\n", name, nameCompany, vacancy, salary);
            this.salary = salary;
            answer = true;
        }
        else {
            System.out.printf("Специалист %s: я найду работу получше! компания %s; вакансия %s; зар.плата %d)\n", name, nameCompany, vacancy, salary);
        }
    }
    @Override
